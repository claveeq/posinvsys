...
Public Class Payment_Form
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Else
            ...
            For row_num As Integer = 0 To (rowcount) ''''''newww
                If (row_num = (rowcount)) Then
                    Exit For
                Else
                    Dim brcode As String = Checkout_Form.DataGridView1.Rows(row_num).Cells(10).Value
                    Try
                        MysqlConn.Open()
                        Dim Query As String
                        Dim pproduct As String = Checkout_Form.DataGridView1.Rows(row_num).Cells(1).Value
                        Dim qty As Integer = Checkout_Form.DataGridView1.Rows(row_num).Cells(3).Value
                        Dim ptotal As Double = Checkout_Form.DataGridView1.Rows(row_num).Cells(4).Value
                        Query = "INSERT INTO `rmarquez`.`items` (`it_date`, `it_name`, `it_quantity`, `it_price`) VALUES (DATE_FORMAT(NOW(),'%Y-%m-%d') , '" & pproduct & "', '" & qty & "', '" & ptotal & "');"
                        COMMAND = New MySqlCommand(Query, MysqlConn)
                        Reader = COMMAND.ExecuteReader
                        MysqlConn.Close()
                    Catch ex As MySqlException
